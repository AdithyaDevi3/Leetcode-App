...
export async function DELETE(_request: Request, { params }: { params: Promise<{ sessionId: string; jobId: string }> }) {
  try {
    const session = await requireAuth();
    const { sessionId, jobId } = await params;
    const job = await createExecutionJobStore().cancel(jobId, session.user.id, sessionId);
    if (!job) return NextResponse.json({ error: 'Execution job not found or cannot be canceled' }, { status: 404 });
    return NextResponse.json({ jobId: job.id, status: job.status, completedAt: job.completedAt });
  } catch (error) {
    if (error instanceof Error && error.message === 'Unauthorized: Authentication required') return NextResponse.json({ error: 'Authentication required' }, { status: 401 });
    return NextResponse.json({ error: 'Failed to cancel execution job' }, { status: 500 });
  }
}
