...
export function LocalLearnerSettings() {
  ...
  const save = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setMessage(writeLocalLearnerProfile(profile) ? 'Settings saved on this device. Your recommendations are updated.' : 'This browser blocked local storage. Update its site-data settings and try again.'); };

  return <main className="min-h-screen bg-[var(--paper)] px-6 py-8 text-[var(--ink)] sm:py-12"><div className="mx-auto max-w-5xl"><SiteNavigation currentPath="/settings" /><section className="mx-auto mt-10 max-w-2xl rounded-xl bg-white p-6 shadow-sm sm:p-8">
    ...
    </fieldset><fieldset className="space-y-4 border-t pt-6"><label className="flex items-start gap-3"><input className="mt-1" checked={profile.diagnosticOptIn} type="checkbox" onChange={(event) => update('diagnosticOptIn', event.target.checked)} /><span>Include an optional diagnostic in my plan.</span></label><label className="flex items-start gap-3"><input className="mt-1" checked={profile.personalizationOptOut} type="checkbox" onChange={(event) => update('personalizationOptOut', event.target.checked)} /><span><span className="font-medium">Turn off personalized recommendations</span><br /><span className="text-sm text-[var(--muted)]">Use the standard curriculum order while keeping your profile on this device.</span></span></label></fieldset><button className="rounded bg-[var(--moss)] px-4 py-2 font-semibold text-white" type="submit">Save settings</button>{message ? <p aria-live="polite" className="text-[var(--ink)]">{message}</p> : null}</form>}
  </section></div></main>;
}
