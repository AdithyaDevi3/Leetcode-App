...
export function SystemDesignWorkspace() {
  ...
  return <main className="min-h-screen px-4 py-8 text-[var(--ink)] sm:px-8 sm:py-12"><div className="mx-auto max-w-5xl">
    ...
    <header className="mb-6 mt-10"><p className="eyebrow">System design practice</p><h1>Build your system design</h1><p className="max-w-2xl text-[var(--muted)]">Choose a design title, then build and review the requirements, architecture, and failure response. This draft saves only in this browser.</p>{hydrated ? <p className="mt-2 text-sm text-[var(--muted)]" role="status">Saved locally{submittedAt ? ` · submitted ${new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(submittedAt))}` : ''}</p> : <p className="mt-2 text-sm text-[var(--muted)]" role="status">Restoring local draft…</p>}</header>
