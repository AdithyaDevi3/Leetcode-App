export const EXECUTION_LANGUAGES = ['python', 'cpp', 'typescript'] as const;
...
export const isExecutionLanguage = (value: unknown): value is ExecutionLanguage =>
  typeof value === 'string' && EXECUTION_LANGUAGES.includes(value as ExecutionLanguage);
...
export function validateExecutionRequest(request: ExecutionRequest): string[] {
  ...
  if (!isExecutionLanguage(request.language)) errors.push('Unsupported language');
