...
export type LearnerProfile = { goal: LearnerGoal; targetDate?: string; experience: 'new' | 'some' | 'experienced'; preferredLanguage: 'python' | 'cpp' | 'typescript'; weeklyMinutes: number; timezone: string; accessibilityNotes?: string; diagnosticOptIn: boolean };
...
export function validateLearnerProfile(profile: LearnerProfile): string[] {
  const errors: string[] = [];
  if (!['python', 'cpp', 'typescript'].includes(profile.preferredLanguage)) errors.push('Unsupported preferredLanguage');
