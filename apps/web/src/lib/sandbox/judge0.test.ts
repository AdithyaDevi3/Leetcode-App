...
describe('Judge0 sandbox adapter', () => {
  it('sends bounded execution inputs and maps completed output', async () => {
    ...
    const sandbox = createJudge0Sandbox({ endpoint: 'https://judge.example/', token: 'secret', languageIds: { typescript: 74, python: 71, cpp: 54 }, fetcher });
  ...
  it('maps sandbox timeout responses', async () => {
    const sandbox = createJudge0Sandbox({ endpoint: 'https://judge.example', token: 'secret', languageIds: { typescript: 74, python: 71, cpp: 54 }, fetcher: vi.fn().mockResolvedValue(new Response(JSON.stringify({ status: { id: 5 } }), { status: 201 })) });
