...
const practiceHref = (contentId: string) => `/practice?problem=${encodeURIComponent(fromPersistedContentId(contentId))}`;
const contentLabel = (contentId: string) => {
  const localId = fromPersistedContentId(contentId);
  return practiceItems.find((item) => item.id === localId)?.label ?? localId.replace(/[-_]+/g, ' ').replace(/\b\w/g, (letter) => letter.toUpperCase());
};
