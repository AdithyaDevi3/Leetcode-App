...
import { getOwnedRuntimeJob } from '@/lib/evaluation-job-runtime';
import { listRuntimeAppeals, submitRuntimeAppeal } from '@/lib/evaluation-appeal-runtime';
...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const job = await getOwnedRuntimeJob(jobId, session.user.id, sessionId);
    ...
    const appeal = await submitRuntimeAppeal({ jobId, userId: session.user.id, findingId: body.findingId, context: body.context.trim() });
...
export async function GET(_request: Request, { params }: Context) {
  try {
    const session = await requireAuth(); const { sessionId, jobId } = await params; const job = await getOwnedRuntimeJob(jobId, session.user.id, sessionId);
    if (!job) return NextResponse.json({ error: 'Evaluation job not found' }, { status: 404 });
    return NextResponse.json({ appeals: await listRuntimeAppeals(jobId, session.user.id) });
  } catch (error) {
    if (error instanceof Error && error.message === 'Unauthorized: Authentication required') return NextResponse.json({ error: 'Authentication required' }, { status: 401 });
