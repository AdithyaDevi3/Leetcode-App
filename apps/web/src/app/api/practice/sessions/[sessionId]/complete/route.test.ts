...
describe('/api/practice/sessions/[sessionId]/complete', () => {
  ...
  it('rejects a direct completion claim without verified code evidence', async () => {
  ...
  it('allows a learner to clear completion while revising', async () => {
    getPracticeOwner.mockResolvedValue({ kind: 'user', id: 'user-1' });
    completePracticeSession.mockResolvedValue({ session: { id: 'session-1', status: 'in_progress' } });
    const { POST } = await import('./route');
    const response = await POST(
      new Request('http://localhost/api/practice/sessions/session-1/complete', {
        method: 'POST',
        body: JSON.stringify({ completed: false, currentStage: 'implement' }),
      }),
      { params: Promise.resolve({ sessionId: 'session-1' }) },
    );
