...
export default async function InstructorClassesPage({ searchParams }: {
  searchParams: Promise<{ error?: string }>;
}) {
  const [access, notice] = await Promise.all([
    readInstructorData(async (repository) =>
  ...
  return <main><div className="mx-auto max-w-6xl space-y-7">
    ...
    <AdminSection title="Your classes" description="Open a class to view its code, learners, assignments, and completion counts.">
      {access.data.length === 0 ? <AdminEmptyState>No classes have been created.</AdminEmptyState> : <div className="grid gap-px bg-[var(--line)] sm:grid-cols-2">
        {access.data.map((classroom) => <a className="block bg-[var(--surface)] p-5 no-underline hover:bg-[var(--moss-soft)] focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--moss)]" href={`/teach/${classroom.id}`} key={classroom.id}>
