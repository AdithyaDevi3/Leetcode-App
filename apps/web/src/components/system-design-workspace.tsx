...
export function SystemDesignWorkspace() {
  ...
  return <main className="min-h-screen px-4 py-8 text-[var(--ink)] sm:px-8"><div className="mx-auto max-w-5xl">
    <header className="mb-6"><p className="eyebrow">System design practice</p><h1>Design a reliable service</h1><p className="max-w-2xl text-[var(--muted)]">Build a structured design using ordinary forms and lists. This is a local practice draft only: it is not saved, submitted, or shared.</p></header>
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <section aria-labelledby="validation-heading" className="rounded-lg border border-[var(--line)] bg-white p-5"><h2 id="validation-heading" className="text-lg font-bold">Review feedback</h2>{errors.length ? <div className="mt-3 rounded border border-[var(--coral)] bg-[var(--coral-soft)] p-4" role="alert"><p className="font-semibold">Complete these items before calling the draft ready:</p><ul className="mt-2 list-disc space-y-1 pl-5">{errors.map((error) => <li key={error}>{error}</li>)}</ul></div> : <p className="mt-3 rounded border border-[var(--moss)] bg-[var(--moss-soft)] p-4 font-semibold" role="status">Your practice design has all required sections.</p>}<p className="mt-4 text-sm text-[var(--muted)]">Validation is instant and local. It does not save or submit this design.</p></section>
    </div>
  </div></main>;
}
