...
import { failStaleExecutionJobs, processNextExecutionJob } from '@/workers/execution-worker';
...
export async function POST(request: Request) {
  ...
  const recoveredFailed = await failStaleExecutionJobs();
  const processed = [];
  ...
  return NextResponse.json({ recoveredFailed, processed });
}
