...
import { ClassroomNotFoundError, PostgresClassroomRepository } from '@leetcode-app/database';
...
export default async function AdminClassDetailPage({ params, searchParams }: {
  params: Promise<{ classId: string }>;
  searchParams: Promise<{ error?: string; created?: string }>;
}) {
  const [{ classId }, notice] = await Promise.all([params, searchParams]);
  const access = await readAdministrationData('classes.read', async (_repository, _principal, db) => {
    try { return await new PostgresClassroomRepository(db).getClassDetail(classId); }
  ...
  return <div className="space-y-7">
    <Link className="inline-flex text-sm font-bold text-[var(--moss)] underline underline-offset-4" href="/admin/classes">← All classes</Link>
    ...
    <AdminSection title="Assign a practice task" description="Choose an existing activity. Completion is based on passing its verified code tests.">
      <form action={createClassAssignment} className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6">
        ...
        <label className="grid gap-1.5 text-sm font-bold">Audit reason
          <input className="min-h-11 rounded-md border border-[var(--line)] bg-white px-3 font-normal focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--moss)]" maxLength={500} minLength={8} name="reason" placeholder="Part of week one practice" required />
        </label>
