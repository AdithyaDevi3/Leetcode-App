...
const requireAuth = vi.fn(); const findOwned = vi.fn(); const cancel = vi.fn();
vi.mock('@/lib/auth/session', () => ({ requireAuth }));
vi.mock('@/lib/execution-jobs-postgres', () => ({ createExecutionJobStore: () => ({ findOwned, cancel }) }));
...
describe('/api/practice/sessions/[sessionId]/execute/[jobId]', () => {
  ...
  it('cancels an owned queued job', async () => {
    requireAuth.mockResolvedValue({ user: { id: 'user-1' } }); cancel.mockResolvedValue({ id: 'job-1', status: 'canceled', completedAt: '2026-08-29T00:00:00Z' });
    const { DELETE } = await import('./route');
    const response = await DELETE(new Request('http://localhost', { method: 'DELETE' }), { params: Promise.resolve({ sessionId: 'session-1', jobId: 'job-1' }) });
    expect(response.status).toBe(200); expect(cancel).toHaveBeenCalledWith('job-1', 'user-1', 'session-1');
  });
});
