...
export function LocalLearnerDashboard() {
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-12 text-slate-900"><section className="mx-auto max-w-3xl rounded-xl bg-white p-8 shadow-sm">
    ...
    {profile && plan ? <div className="mt-6 space-y-7">
      <section aria-labelledby="progress-heading" className="border border-slate-200 bg-slate-50 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold uppercase tracking-wide text-slate-500">Your momentum</p><h2 className="mt-1 text-xl font-bold" id="progress-heading">Progress you can build on</h2></div><Link className="font-semibold underline underline-offset-4" href="/history">View history</Link></div>
        <dl className="mt-5 grid gap-4 sm:grid-cols-4"><div><dt className="text-sm text-slate-500">Completed</dt><dd className="text-2xl font-bold">{progress?.completedItems ?? 0}</dd></div><div><dt className="text-sm text-slate-500">Active days</dt><dd className="text-2xl font-bold">{progress?.activeDays ?? 0}</dd></div><div><dt className="text-sm text-slate-500">Current streak</dt><dd className="text-2xl font-bold">{progress?.currentStreak ?? 0} <span className="text-sm font-medium">days</span></dd></div><div><dt className="text-sm text-slate-500">Average score</dt><dd className="text-2xl font-bold">{progress?.averageScore === null || progress?.averageScore === undefined ? '—' : `${progress.averageScore}/100`}</dd></div></dl>
        {progress?.completedItems === 0 ? <p className="mt-4 text-sm text-slate-600">Finish one practice item to start tracking your momentum.</p> : null}
      </section>
      <section aria-labelledby="plan-heading">
        <h2 className="text-xl font-bold" id="plan-heading">Recommended algorithms</h2>
        <p className="mt-2 text-slate-700">{plan.explanation}</p>
        <p className="mt-2 text-sm text-slate-600">Pace: <span className="font-semibold capitalize">{plan.pace}</span></p>
        <ol className="mt-4 space-y-3">
          {plan.recommendations.map((recommendation, index) => <li className="border border-slate-200 p-4" key={recommendation.practiceItemId}>
            <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-semibold text-slate-500">{index === 0 ? 'Next up' : `Option ${index + 1}`}</p><h3 className="font-bold">{recommendation.label}</h3></div><span className="text-sm text-slate-600">{recommendation.estimatedMinutes} min · {recommendation.difficulty}</span></div>
            <p className="mt-2 text-sm text-slate-600">{recommendation.reasons.join(' · ')}</p>
          </li>)}
        </ol>
        {nextPractice ? <a className="mt-5 inline-block rounded bg-slate-900 px-4 py-2 font-semibold text-white" href={practiceHref(nextPractice.practiceItemId)}>Start {nextPractice.label}</a> : null}
      </section>
